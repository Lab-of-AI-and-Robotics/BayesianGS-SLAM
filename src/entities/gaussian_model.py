...
class GaussianModel:
    def __init__(self, sh_degree: int = 3, isotropic=False):
        self.gaussian_param_names = [
            ...
            "rotation",
            "opacity",
            "color_variance",  # Add learnable color variance
            "max_radii2D",
            "xyz_gradient_accum",
            "denom",
            "spatial_lr_scale",
        ...
        self._color_variance = torch.empty(0).cuda()  # Initialize color variance parameter
    ...
    def restore_from_params(self, params_dict, training_args):
        ...
        color_variance = params_dict.get("color_variance", None)
    ...
    def setup_functions(self):
        ...
        # Color variance activation: ensures positive values and reasonable scale
        # self.color_variance_activation = lambda x: torch.exp(x)  # Always positive
        # self.inverse_color_variance_activation = torch.log
        self.color_variance_activation = F.softplus
        self.inverse_color_variance_activation = lambda x: torch.log(torch.expm1(x))
    ...
    def capture_dict(self):
        return {
            ...
            "color_variance": self._color_variance.clone().detach().cpu(),
    ...
    def get_color_variance(self):
        """Get color variance for each Gaussian (diagonal of 3x3 covariance).
        Returns learnable color variance parameters with exponential activation for positivity."""
        return self.color_variance_activation(self._color_variance)
    ...
    def add_points(self, pcd: o3d.geometry.PointCloud, global_scale_init=True):
        ...
        # Initialize color variance for new points
        n_new_points = fused_point_cloud.shape[0]
        new_color_variance = torch.full((n_new_points, 1), VALUE_INIT, device="cuda", dtype=torch.float32)
        new_color_variance = nn.Parameter(new_color_variance.requires_grad_(True))
    ...
    def training_setup(self, training_args, exposure_ab=None):
        ...
        params = [
            ...
            {"params": [self._color_variance], "lr": training_args.feature_lr, "name": "color_variance"},
        ]
    ...
    def training_setup_camera(self, cam_rot, cam_trans, cfg, exposure_ab=None):
        ...
        params = [
            ...
            {"params": [self._color_variance], "lr": 0.0, "name": "color_variance"},
    ...
    def construct_list_of_attributes(self):
        ...
        # Add color variance attributes
        for i in range(self._color_variance.shape[1]):  # Should be 3 for RGB
            l.append("color_var_{}".format(i))
        return l
    ...
    def save_ply(self, path):
        ...
        color_variance = self._color_variance.detach().cpu().numpy()
        attributes = np.concatenate((xyz, normals, f_dc, f_rest, opacities, scale, rotation, color_variance), axis=1)  
    ...
    def load_ply(self, path):
        ...
        # Load color variance (with backward compatibility)
        color_var_names = [p.name for p in plydata.elements[0].properties if p.name.startswith("color_var_")]
        if color_var_names:  # Check if color variance exists
            color_var_names = sorted(color_var_names, key=lambda x: int(x.split("_")[-1]))
            color_variances = np.zeros((xyz.shape[0], len(color_var_names)))
            for idx, attr_name in enumerate(color_var_names):
                color_variances[:, idx] = np.asarray(plydata.elements[0][attr_name])
        else:
            # Default color variance for older PLY files (log space, so 0 = variance of 1.0)
            print("No color variance found, using default value")
            color_variances = np.full((xyz.shape[0], 1), 0.0)
        
        self._color_variance = nn.Parameter(torch.tensor(color_variances, dtype=torch.float, device="cuda").requires_grad_(True))
    ...
    def prune_points(self, mask):
        ...
        self._color_variance = optimizable_tensors["color_variance"]
    ...
    def densification_postfix(self, new_xyz, new_features_dc, new_features_rest,
                              new_opacities, new_scaling, new_rotation, new_color_variance=None):
        d = {
            "xyz": new_xyz,
        ...
        # Handle color variance parameter
        if new_color_variance is not None:
            d["color_variance"] = new_color_variance
        else:
            # For backward compatibility, create zeros if not provided
            n_points = new_xyz.shape[0]
            default_color_variance = torch.full((n_points, 1), VALUE_INIT, device="cuda", dtype=torch.float32)
            d["color_variance"] = nn.Parameter(default_color_variance.requires_grad_(True))
    ...
    def create_from_pcd(self, pcd: BasicPointCloud, spatial_lr_scale: float):
        ...
        # Initialize color variance parameters
        # Start with unit variance (log space): exp(0) = 1.0 variance for training
        n_points = fused_point_cloud.shape[0]
        initial_color_variance = torch.full((n_points, 1), VALUE_INIT, device="cuda", dtype=torch.float32)
        self._color_variance = nn.Parameter(initial_color_variance.requires_grad_(True))
