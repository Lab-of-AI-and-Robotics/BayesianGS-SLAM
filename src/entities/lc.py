...
class Loop_closure(object):
    def __init__(self, config: dict, dataset: BaseDataset, logger: Logger, output_path: Path) -> None:
        ...
        Args:
            ...
            output_path: Path to the output directory for saving loop closure results.
        """
        self.device = "cuda"
        ...
        self.submap_gaussians = dict()


        self.total_reg = []

        # Initialize log file
        log_file_path = Path(self.logger.output_path) / "lc.log"
    ...
    def __del__(self):
        """Close log file when lc is destroyed."""
        if hasattr(self, 'log_file') and self.log_file:
            self.log_file.close()

    def submap_reliability(self, gaussian_model, eps=1e-8):
        # alpha_i: [N] (opacity/importance). If you don’t have it, use ones_like(w_i).
        # if alpha_i is None: alpha_i = torch.ones_like(w_i)
        var_i = gaussian_model.get_color_variance()
        alpha_i = gaussian_model.get_opacity().squeeze(1)

        var_i = var_i.mean(dim=1)
        logv = (var_i + eps).log()
        mu = logv.median()
        w = torch.exp(-(logv - mu)/self.tau_lc)
        w = w / (w.mean() + eps)                  # center ~1
        # w_i = torch.clamp(w, *clamp)
        w_i = torch.clamp(w, min=1/self.w_limit, max=self.w_limit)

        return (w_i * alpha_i).sum() / (alpha_i.sum() + eps)  # scalar ~1


    def update_submaps_info(self, keyframes_info, gaussian_model=None, all_frame_ids=None):
        """Update the submaps_info with current submap

        Args:
            keyframes_info (dict): a dictionary of all submap information for loop closures
            gaussian_model: GaussianModel for this submap (for reliability scoring)
            all_frame_ids (list): all frame IDs in this submap (for loop detection descriptors).
                                  If None, falls back to keyframe IDs only.
        """
        with torch.no_grad():
            # 모든 프레임에서 descriptor 추출 (루프 탐지 + self_sim)
            desc_ids = sorted(all_frame_ids) if all_frame_ids is not None else sorted(keyframes_info.keys())
            all_desc = []
            for fid in desc_ids:
                all_desc.append(self.netvlad(np2torch(self.dataset[fid][1], self.device).permute(2, 0, 1)[None]/255.0))
            all_desc = torch.cat(all_desc)

            self_sim = torch.einsum("id,jd->ij", all_desc, all_desc)
            score_min, _ = self_sim.topk(max(int(len(all_desc) * self.config["lc"]["min_similarity"]), 1))

            # 키프레임 descriptor만 별도 추출 (정합 viewpoint 선택용)
            kf_ids_sorted = sorted(keyframes_info.keys())
            kf_set = set(kf_ids_sorted)
            kf_indices = [i for i, fid in enumerate(desc_ids) if fid in kf_set]
            kf_desc = all_desc[kf_indices]

        self.submap_lc_info[self.submap_id] = {
                "submap_id": self.submap_id,
                "kf_id": np.array(kf_ids_sorted),
                "kf_desc": kf_desc,              # 키프레임만 (정합 viewpoint 선택용)
                "all_desc": all_desc,             # 모든 프레임 (루프 탐지용)
                "self_sim": score_min,
            }
        self.submap_gaussians[self.submap_id] = gaussian_model
    ...
    def submap_loader(self, id: int):
        ...
        for kf_id in submap_dict['submap_keyframes']:
            ...
            cam_i = Camera(kf_id, None, None,
                   ...
                   self.config["cam"]["H"], 
                   self.config["cam"]["W"])
            cam_i.R = T_est[:3, :3].detach().clone()
            cam_i.T = T_est[:3, 3].detach().clone()
    ...
    def detect_closure(self, query_id: int, final=False):
        ...
        # current_gaussian_model = self.submap_gaussians[query_id]
        ...
        for db_info in db_info_list:
            db_desc_map_id += [db_info['submap_id'] for _ in db_info['all_desc']]
        db_desc_map_id = torch.Tensor(db_desc_map_id).to(self.device)

        query_desc = query_info['all_desc']
        db_desc = torch.cat([db_info['all_desc'] for db_info in db_info_list])

        with torch.no_grad():
            
            # r_q_single = self.submap_reliability(current_gaussian_model)
            # r_db_lst = []
            # for db_info in db_info_list:
                # r_db_lst.append(self.submap_reliability(self.submap_gaussians[db_info['submap_id']]))

            # r_q = torch.full((query_desc.shape[0],), float(r_q_single), device=query_desc.device)

            # r_db_kf = torch.cat([
                # torch.full((db['kf_desc'].shape[0],), float(r_db_lst[d_id]), device=db['kf_desc'].device)
                # for d_id, db in enumerate(db_info_list)
            # ]) 

            # beta, gmin, gmax = 1.0, 0.85, 1.20
            # G = torch.clamp((r_q[:, None] * r_db_kf[None, :]).pow(beta), gmin, gmax).unsqueeze(0)  # [1, M]

            cross_sim = torch.einsum("id,jd->ij", query_desc, db_desc)
            cross_sim_gated = cross_sim #* G

            self_sim = query_info['self_sim']
            matches = torch.argwhere(cross_sim_gated > self_sim[:,[-1]])[:,-1]
        ...
        for source_id in tqdm(reversed(range(1, n_submaps))):
            ...
            for target_id in iterator:
                if abs(target_id - source_id)== 1: # odometry edge
                    reg_dict = self.pairwise_registration(submap_list[source_id], submap_list[target_id], "identity", frame_id=source_id, tgt_id=target_id)
                    ...
                    pose_graph.edges.append(
                        o3d.pipelines.registration.PoseGraphEdge(source_id,
                                                                ...
                                                                information,
                                                                uncertain=False))
                    gt_dict = self.pairwise_registration(submap_list[source_id], submap_list[target_id], "gt", frame_id=source_id, tgt_id=target_id)
                    ...
                    reg_dict = self.pairwise_registration(submap_list[source_id], submap_list[target_id], "gs_reg", frame_id=source_id, tgt_id=target_id)
                    toc = time.perf_counter()
                    lc_time = toc - tic
                    self.total_lc_time += lc_time
                    self._log(f"Loop closure time: {lc_time} seconds")
    ...
    def loop_closure(self, estimated_c2ws, final=False):
        ...
        if len(loop_edges)+len(odometry_edges) > 0:
            self._log(f'Average edge registration time: {total_reg / (len(loop_edges)+len(odometry_edges))} seconds')
            if len(loop_edges) > 0:
                self._log(f'Average loop closure time: {self.total_lc_time / len(loop_edges)} seconds')
        else:
            self._log(f'No edge detected')
        
        # save pgo edge analysis result
        
        if len(loop_edges)>0 and len(loop_edges) > self.n_loop_edges: 
            
            self._log("Optimizing PoseGraph ...")
            ...
            self._log("No valid loop edges or new loop edges. Skipping ...")
    ...
    def analyse_pgo(self, odometry_edges, loop_edges, pose_graph):
        ...
        """
        pgo_save_path = self.output_path / "pgo" / str(self.pgo_count)
        pgo_save_path.mkdir(parents=True, exist_ok=True)
        
        self._log("Evaluating ATE before pgo ...")
        ...
        self._log("Evaluating ATE after pgo ...")
        ...
        self._log("Evaluating ATE within each submap ...")
    ...
    def pairwise_registration(self, submap_source, submap_target, method="gs_reg", frame_id=None, tgt_id=None):
        ...
        if method == "gt":
            ...
            res = gs_reg(submap_source, submap_target, self.config['lc']['registration'], config_var=self.config['var'])
            delta = res['pred_tsfm'].cpu().numpy()
            output["transformation"] = delta
            output["successful"] = res["successful"]
            # if res["successful"] is True:
            #     self.logger.vis_lc_registration(frame_id, tgt_id, renders=res["renders"])
