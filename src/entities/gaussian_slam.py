""" Main VarSplat SLAM module. Controls Mapper, Tracker, and Loop Closure modules.
    Manages submap lifecycle and camera pose estimation updates.
"""
import json
import os
import pprint
...
from src.utils.utils import (np2torch, setup_seed, torch2np, get_gpu_memory_stats, 
                             format_memory_mb, reset_peak_gpu_memory)
from src.utils.vis_utils import *  # noqa - needed for debugging
import time

class Sensor_uncertainty_MLP(nn.Module):
    def __init__(self, D=4, input_ch=1, output_ch=1, hidden_ch=32, leaky=False, beta=1e-2):
        super(Sensor_uncertainty_MLP, self).__init__()
        self.input_ch = input_ch
        self.output_ch = output_ch
        self.hidden_ch = hidden_ch
        self.beta = beta
        
        self.noise_conv = nn.ModuleList(
            [nn.Conv2d(self.input_ch, self.hidden_ch, 3, padding=1)] + 
            [nn.Conv2d(self.hidden_ch, self.hidden_ch, 3, padding=1) for i in range(D-1)]
        )
        
        self.noise_bn = nn.ModuleList(
            [nn.GroupNorm(4, self.hidden_ch) for i in range(D)]
        )
        
        self.last_conv = nn.Sequential(
            nn.Conv2d(self.hidden_ch, self.hidden_ch, kernel_size=1),
            # nn.GroupNorm(4, self.hidden_ch), 
            ...
            nn.Conv2d(self.hidden_ch, self.output_ch, kernel_size=1)
        )
        
        self.uncert_act_evidential = nn.Softplus()
    ...
    def forward(self, x):
        if len(x.shape) == 2:
            x = x.unsqueeze(0)
        for i, l in enumerate(self.noise_conv):
            # x = self.act(self.noise_bn[i](l(x)))
            x = self.act(l(x))
            
        x = self.last_conv(x)
        return self.uncert_act_evidential(x.squeeze()) + self.beta
...
class GaussianSLAM(object):
    ...
    def __init__(self, config: dict) -> None:
        ...
        self.use_nlldepth = config["var"]["use_nlldepth"]
        ...
        # submap_id → 해당 submap의 첫 번째 프레임 ID
        # submap 0은 항상 frame 0에서 시작
        self.submap_start_frames = {0: 0}

        self.sensor_uncertainty_MLP = Sensor_uncertainty_MLP(input_ch=4, output_ch=2, beta=0.0001).to("cuda")
        self.sensor_uncertainty_MLP.share_memory()

        self.logger = Logger(self.output_path, config["use_wandb"], timing_log_path=self.timing_log_path)
        self.mapper = Mapper(config["mapping"], self.dataset, self.logger, config["var"], self.sensor_uncertainty_MLP)
        self.tracker = Tracker(config["tracking"], self.dataset, self.logger, config["var"], self.sensor_uncertainty_MLP)
        self.enable_exposure = self.tracker.enable_exposure
        self.loop_closer = Loop_closure(config, self.dataset, self.logger, self.output_path)
        self.loop_closer.submap_path = self.output_path / "submaps"
        
        self.log_and_print('Tracking config')
        tracking_config_str = pprint.pformat(config["tracking"])
        self.log_and_print(tracking_config_str)
        self.log_and_print('Mapping config')
        mapping_config_str = pprint.pformat(config["mapping"])
        self.log_and_print(mapping_config_str)
        self.log_and_print('Loop closure config')
        lc_config_str = pprint.pformat(config["lc"])
        self.log_and_print(lc_config_str)
        self.log_and_print(f"⭐️ INFO: Timing and GPU memory log can be found at {self.timing_log_path}")
        self.keyframe_count = config["keyframe_count"] if "keyframe_count" in config else 1
        
    def log_and_print(self, message, end='\n'):
        """
        Write message to both log file and print to console.
        
        Args:
            message: Message to log
            end: End character for the message (default: newline)
        """
        if hasattr(self, 'timing_log_path'):
            # Append to file
            try:
    ...
    def _setup_output_path(self, config: dict) -> None:
        ...
        if "output_path" in config["data"]:
            ...
            self.output_path = Path("results") / self.timestamp
        ...
        self.timing_log_path = self.output_path / "timing_gpu_log.txt"
        # Initialize log file with header
        with open(self.timing_log_path, 'w') as f:
            f.write("="*60 + "\n")
            f.write("VarSplat Timing and GPU Memory Log\n")
            f.write("="*60 + "\n\n")
    ...
    def start_new_submap(self, frame_id: int, gaussian_model: GaussianModel) -> None:
        ...
        self.submap_id += 1
        self.submap_start_frames[self.submap_id] = frame_id  # 새 submap 시작 프레임 기록
    ...
    def update_keyframe_poses(self, lc_output, submaps_kf_ids, cur_frame_id):
        '''
        Update all frame poses (keyframe and non-keyframe) using PGO correction.
        Each submap's correction is applied to every frame in that submap's actual
        frame range, not just the keyframe range, to prevent pose discontinuities.
        '''
        for correction in lc_output:
            submap_id = correction['submap_id']
            correct_tsfm = correction['correct_tsfm']

            # 전체 submap 프레임 범위: 이 submap의 첫 프레임 ~ 다음 submap의 첫 프레임 - 1
            # 마지막 submap은 cur_frame_id까지 포함
            start_id = self.submap_start_frames[submap_id]
            if submap_id + 1 in self.submap_start_frames:
                end_id = self.submap_start_frames[submap_id + 1] - 1
            else:
                end_id = cur_frame_id

            self.estimated_c2ws[start_id:end_id + 1] = (
                torch.from_numpy(correct_tsfm).float() @ self.estimated_c2ws[start_id:end_id + 1]
            )
    ...
    def run(self) -> None:
        """ Starts the main VarSplat SLAM pipeline, including tracking, mapping, and loop closure. """
        setup_seed(self.config["seed"])
        
        # Start overall runtime tracking
        total_start_time = time.perf_counter()
        
        # Initialize GPU memory tracking
        initial_gpu_stats = None
        if torch.cuda.is_available():
            # Reset peak memory stats before starting
            device = self.device
            if 'cuda' in str(device):
                reset_peak_gpu_memory(device)
                initial_gpu_stats = get_gpu_memory_stats(device)
        
        gaussian_model = GaussianModel(0)
        gaussian_model.training_setup(self.opt)
        self.submap_id = 0
        color_keyframe_select_loss = 9999999
        depth_keyframe_select_loss = 9999999
        total_keyframe_count = 0
        keyframe_count = 0
        keyframe_count_log = {}  # frame_id -> cumulative keyframe count

        current_submap_frames = []
        ...
        for frame_id in range(len(self.dataset)):
            ...
            if frame_id in [0, 1]:
                ...
                estimated_c2w, exposure_ab, color_loss, depth_loss, depth_keyframe_select_loss, color_keyframe_select_loss = self.tracker.track(
            ...
            if self.should_start_new_submap(frame_id):
                ...
                # 루프 탐지에는 모든 프레임 descriptor 사용, 정합에는 키프레임만 사용
                self.loop_closer.update_submaps_info(self.keyframes_info, gaussian_model,
                                                     all_frame_ids=current_submap_frames)
                current_submap_frames = []
            ...
            current_submap_frames.append(frame_id)

            # if frame_id in self.mapping_frame_ids:
            keyframe_count = keyframe_count+1
            if depth_keyframe_select_loss > 0 or (not self.use_nlldepth and color_keyframe_select_loss > 0) or not bool(self.keyframes_info) or keyframe_count > self.keyframe_count:
                keyframe_count = 0
                if self.use_nlldepth:
                    self.log_and_print(f"\nMapping frame {frame_id}, depth_keyframe_select_loss: {depth_keyframe_select_loss:.4f}")
                else:
                    self.log_and_print(f"\nMapping frame {frame_id}, color_keyframe_select_loss: {color_keyframe_select_loss:.4f}, depth_keyframe_select_loss: {depth_keyframe_select_loss:.4f}")
                ...
                total_keyframe_count += 1
                keyframe_count_log[frame_id] = total_keyframe_count
        ...
        with open(self.output_path / "keyframe_count_log.json", "w") as f:
            json.dump({"frame_id": list(keyframe_count_log.keys()),
                       "cumulative_keyframe_count": list(keyframe_count_log.values())}, f, indent=2)
        
        # End overall runtime tracking
        total_runtime = time.perf_counter() - total_start_time
        
        # Get final GPU memory stats
        final_gpu_stats = None
        if torch.cuda.is_available():
            device = self.device
            if 'cuda' in str(device):
                final_gpu_stats = get_gpu_memory_stats(device)
        
        # Print and log overall timing summary
        log_lines = [
            f"\n{'='*60}",
            f"Overall Runtime Summary:",
            f"  Total runtime: {total_runtime:.4f} seconds ({total_runtime/60:.2f} minutes)"
        ]
        
        n_frames = len(self.dataset)
        if n_frames > 0:
            overall_fps = n_frames / total_runtime
            log_lines.extend([
                f"  Total frames processed: {n_frames}",
                f"  Average FPS: {overall_fps:.2f}"
            ])
        
        # Overall GPU memory summary
        if final_gpu_stats and torch.cuda.is_available():
            log_lines.append(f"\n  Overall GPU Memory Usage:")
            log_lines.append(f"    Peak allocated memory: {format_memory_mb(final_gpu_stats['max_allocated_mb'])}")
            log_lines.append(f"    Peak reserved memory: {format_memory_mb(final_gpu_stats['max_reserved_mb'])}")
            if initial_gpu_stats:
                memory_increase = final_gpu_stats['max_allocated_mb'] - initial_gpu_stats['allocated_mb']
                log_lines.append(f"    Memory increase from start: {format_memory_mb(memory_increase)}")
            log_lines.append(f"    Final allocated memory: {format_memory_mb(final_gpu_stats['allocated_mb'])}")
            log_lines.append(f"    Final reserved memory: {format_memory_mb(final_gpu_stats['reserved_mb'])}")
