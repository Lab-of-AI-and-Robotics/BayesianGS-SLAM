...
def save_var_heatmap_with_colorbar(var_np: np.ndarray, save_path: str, label: str = "log10(variance)") -> None:
    """Save a 2D variance map as a heatmap image with a colorbar showing actual values."""
    var_log = np.log10(var_np + 1e-8)
    vmin, vmax = float(var_log.min()), float(var_log.max())
    var_normalized = (var_log - vmin) / (vmax - vmin + 1e-8)

    cmap = cm.get_cmap('turbo')
    heatmap_rgb = cmap(var_normalized)[:, :, :3]

    fig, (ax_img, ax_cb) = plt.subplots(
        1, 2, figsize=(7, 4),
        gridspec_kw={"width_ratios": [10, 0.5]}
    )
    ax_img.imshow(heatmap_rgb, aspect='auto')
    ax_img.axis('off')
    ...
    cbar.set_ticklabels([f"{10**v:.2e}" for v in tick_log], fontsize=7)
...
class Mapper(object):
    def __init__(self, config: dict, dataset: BaseDataset, logger: Logger, config_var: dict, sensor_uncertainty_MLP: nn.Module) -> None:
        ...
        self.mlp_optimizer = torch.optim.Adam(                                                                         
            self.sensor_uncertainty_MLP.parameters(), lr=1e-4                                                          
        )
    ...
    def optimize_submap(self, keyframes: list, gaussian_model: GaussianModel, iterations: int = 100) -> dict:
        ...
        while iteration < iterations + 1:
            ...
            if train_bayesian:
                color_variance = render_pkg["render_color_variance"]
                depth_variance = render_pkg["render_depth_variance"]
                eps = 1e-6
                log_weight = 0.5
                

                input_data = torch.cat([gt_depth.unsqueeze(0), gt_image], dim=0)
                # input_data = gt_depth
                sensor_uncert = self.sensor_uncertainty_MLP(input_data)
                # total_color_uncert = sensor_uncert.unsqueeze(0) + color_variance
                # total_depth_uncert = sensor_uncert.unsqueeze(0) + depth_variance
                # total_color_uncert = total_color_uncert*1e+3
                # total_depth_uncert = total_depth_uncert*1e+3
                
                total_color_uncert = sensor_uncert[0:1,:,:] + color_variance
                total_depth_uncert = sensor_uncert[1:2,:,:] + depth_variance
                
                resid2 = (image[:, mask] - gt_image[:, mask]) ** 2
                resid2_d = (depth[:, mask] - gt_depth[mask]) ** 2
                
                # print("setet")
                # print(total_uncert.shape)
                # print(image.shape)

                nll = 0.5 * (resid2 / (total_color_uncert[:, mask])) + log_weight * torch.log(total_color_uncert[:, mask] )
                nll_d = 0.5 * (resid2_d / (total_depth_uncert[:, mask] )) + log_weight * torch.log(total_depth_uncert[:, mask] )

                nll_color = nll.mean()
                nll_depth = nll_d.mean()
                
                if self.use_nlldepth:
                    total_loss = depth_loss + reg_loss + color_loss + 0.0001 * (nll_depth)
                else:
                    total_loss = depth_loss + reg_loss + color_loss + 0.0001 * (nll_depth + nll_color)
    ...
    def map(self, frame_id: int, estimate_c2w: np.ndarray, gaussian_model: GaussianModel, is_new_submap: bool, exposure_ab=None) -> dict:
        ...
        self.sensor_uncertainty_MLP.train()
        ...
        # with torch.no_grad():
            ...
            # sensor_variance_vis = True
            
            # (self.logger.output_path / "sensor_var_vis").mkdir(exist_ok=True, parents=True)
            # # var_magnitude = self.sensor_uncertainty_MLP(keyframe["depth"])
            # var_magnitude = self.sensor_uncertainty_MLP(  torch.cat([keyframe["depth"].unsqueeze(0), keyframe["color"]], dim=0)  )
            # var_magnitude_color = var_magnitude[0:1,:,:].squeeze(0)
            # var_magnitude_depth = var_magnitude[1:2,:,:].squeeze(0)
            # var_np_color = var_magnitude_color.detach().cpu().numpy()
            # var_np_depth = var_magnitude_depth.detach().cpu().numpy()
            # fig_name = str(self.logger.output_path / "sensor_var_vis" / f'color_{frame_id:04d}.png')
            # fig_name_depth = str(self.logger.output_path / "sensor_var_vis" / f'depth_{frame_id:04d}.png')
            # save_var_heatmap_with_colorbar(var_np_color, fig_name, label="sensor uncertainty")
            # save_var_heatmap_with_colorbar(var_np_depth, fig_name_depth, label="sensor uncertainty")
            
            # epis_color_variance_vis = render_pkg_vis.get("render_color_variance", None)
            # (self.logger.output_path / "epis_color_var_vis").mkdir(exist_ok=True, parents=True)
            # var_np = epis_color_variance_vis.mean(dim=0).detach().cpu().numpy()  # [3,H,W] -> [H,W]
            # fig_name = str(self.logger.output_path / "epis_color_var_vis" / f'variance_{frame_id:04d}.png')
            # save_var_heatmap_with_colorbar(var_np, fig_name, label="epistemic color variance")

            # epis_depth_variance_vis = render_pkg_vis.get("render_depth_variance", None)
            # (self.logger.output_path / "epis_depth_var_vis").mkdir(exist_ok=True, parents=True)
            # var_np = epis_depth_variance_vis.squeeze(0).detach().cpu().numpy()  # [1,H,W] -> [H,W]
            # fig_name = str(self.logger.output_path / "epis_depth_var_vis" / f'variance_{frame_id:04d}.png')
            # save_var_heatmap_with_colorbar(var_np, fig_name, label="epistemic depth variance")
            # if sensor_variance_vis is not None:
            #     try:
            #     except Exception:
            #         pass
