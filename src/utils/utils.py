...
def render_gaussian_model(gaussian_model, render_settings,
    ...
    if return_variance:
        color, depth, alpha, radii, variance, render_color_variance, render_depth_variance = result
        no_gs_mask = (alpha < 0.6)  # [1, H, W]: pixels with no Gaussian coverage
        render_color_variance = (render_color_variance + 1e-3).masked_fill(no_gs_mask, 100.0)
        render_depth_variance = (render_depth_variance + 1e-3).masked_fill(no_gs_mask, 100.0)
        return {"color": color, "depth": depth, "radii": radii, "means2D": means2D, "alpha": alpha, "variance": variance, "render_color_variance": render_color_variance, "render_depth_variance": render_depth_variance}
    else:
