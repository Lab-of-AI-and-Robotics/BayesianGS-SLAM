...
def merge_submaps(submaps_paths: list, radius: float = 0.0001, device: str = "cuda") -> o3d.geometry.PointCloud:
    ...
    for submap_path in tqdm(submaps_paths, desc="Merging submaps"):
        ...
        print(f"There are {len(current_pts)} points in the current submap")
...
def refine_global_map(pt_cloud: o3d.geometry.PointCloud, training_frames: list, max_iterations: int,
    ...
    for iteration in tqdm(range(max_iterations), desc="Refinement"):
        ...
        ###########
        color_variance = render_dict["render_color_variance"].permute(1, 2, 0)
        depth_variance = render_dict["render_depth_variance"]
        log_weight = 0.5
        total_color_uncert = color_variance[depth_mask, :]
        total_depth_uncert = depth_variance[:, depth_mask]
        
        resid2 = (rendered_color[depth_mask, :] - gt_color[depth_mask, :]) ** 2
        resid2_d = (rendered_depth[:, depth_mask] - gt_depth[depth_mask]) ** 2

        nll = 0.5 * (resid2 / (total_color_uncert)) + log_weight * torch.log(total_color_uncert)
        nll_d = 0.5 * (resid2_d / (total_depth_uncert )) + log_weight * torch.log(total_depth_uncert)
        ...
        total_loss = color_loss + depth_loss + reg_loss  + 0.0001 * (nll_color + nll_depth)
        total_loss.backward()
